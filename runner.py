...
def run_batch_publishing_job(configs_to_publish=asterisk_tuple, envs_to_publish=asterisk_tuple,
                             ...
                             services_to_publish=asterisk_tuple, cleanup_services=False, config_dir=default_config_dir,
                             log_to_file=True,
                             log_dir=default_log_dir):
    root_logger = setup_console_logger()

    configs = get_configs(configs_to_publish)
...
def run_batch_cleanup_job(configs_to_clean=asterisk_tuple, envs_to_clean=asterisk_tuple,
                          instances_to_clean=asterisk_tuple,
                          log_to_file=True,
                          log_dir=default_log_dir):
    root_logger = setup_console_logger()

    configs = get_configs(configs_to_clean)
    ...
    for config_name, config in configs.iteritems():
        log_file_handler = setup_file_logger(root_logger, config_name, log_dir) if log_to_file else None
        try:
            publisher.cleanup_config(config, included_envs=envs_to_clean,
                                     included_instances=instances_to_clean)
        except Exception:
